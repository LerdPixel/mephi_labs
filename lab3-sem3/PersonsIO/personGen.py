...
male_hobbies = ('Sports', 'Gaming', 'Fishing', 'Hiking', 'Chess', 'Cooking', 'Racing')
female_hobbies = ('Reading', 'Yoga', 'Painting', 'Dancing', 'Cooking', 'Traveling')
...
def generate_hobby(gender):
    if (random.random() > 0.7):
        if gender == 'Male':
            return fake.random_element(elements=male_hobbies)
        else:
            return fake.random_element(elements=female_hobbies)
    else:
        if gender == 'Female':
            return fake.random_element(elements=male_hobbies)
        else:
...
# Function to generate city based on country
def generate_city(country):
    if country == 'United States':
        return fake.city_name() + ', ' + fake.state_abbr()
    else:
        return fake.city() + ', ' + country
...
# Write the list of persons to a text file
with open('person_list_corrected.txt', 'w') as f:
    for person in persons:
        f.write(f"Name: {person['name']}\n")
        f.write(f"Surname: {person['surname']}\n")
        f.write(f"Height: {person['height']} cm\n")
